...
以下 12 项是预期的服务边界。初期可以合并安装，但保留职责和测试边界；Observability 可以按需安装。表中依赖描述完整 v1 的逻辑关系，具体 Nya 必需注入必须按当前已启用能力声明。
...
| K05 ModelService | 注册/撤销模型提供方、能力检查、取得单次调用句柄；首版合并注册和路由 | ModelAdapter SPI、K08 的已解析限制 | 提供方注册和自建客户端；调用资源归 ModelCallScope | G1/G4 |
...
| SPI | 首版实现 | 后续接入 |
| --- | --- | --- |
| ModelAdapter | Mock、OpenAI Responses、Anthropic Messages | Chat Completions、Gemini 及其他协议 |
...
- 适配器承担一次模型调用；本地工具执行与后续模型调用由内核调度。关闭 SDK 自动工具循环，避免绕过权限与执行记录。
...
| Providers | registerModel、registerTool、registerContextProvider | 返回可等待的撤销句柄，按生命周期登记 |
...
- [ ] 固定模型、工具、上下文、StateStore 的首版 SPI，明确取消与完成语义。
...
实现 OpenAI Responses 和 Anthropic Messages 两个独立适配器，使用相同的 ModelAdapter 合约套件。
