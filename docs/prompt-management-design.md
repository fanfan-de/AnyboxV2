...
2. 编辑名称、用途类型、消息角色与草稿内容。编辑请求带 `expectedRevision`；冲突时返回明确错误，用户可以检查差异后重试。类型或角色改变需重新校验内容与可绑定位置。
...
本机 Web 已通过“设置 → Prompt 管理”提供前四项操作。宿主固定操作者为 `local-web-user`，不接受浏览器传入身份；首次编辑内置指令会创建该用户的独立草稿。Web 发布还校验用户看到的草稿修订号，避免旧页面发布后来被改动的内容。编辑器展示当前 Agent 绑定和历史版本，应用旧版本可回退绑定；保存或发布不会自动应用。详见 [Web 协议与流程](./web-client-design.md)。
...
| Agent Prompt 组件 | 接收只读 Agent 定义，注入 Prompt 与数据库服务；校验 Agent 管理权限和版本所有权，持有默认指令与绑定读投影，提供绑定及 Run 版本解析，并等待已接受的绑定写入结束。 |
...
重新生成或编辑历史输入使用新幂等键，在原节点的父位置创建新 Run；它们读取当前 Prompt 与模型配置。原节点及原 Run 的快照不变。同父节点的并发 Run 各自固定快照，不互相读取工具过程。
