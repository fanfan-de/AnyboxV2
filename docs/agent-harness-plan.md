...
| H3 Run 持久化与恢复（现有单步 Run 已完成） | 单实例排他状态提供方、原子记录、关闭后重开、异常退出清算；工具意图记录随 H2 工具循环补充 | 去重与历史可恢复；未完成工具标记不确定；不自动重放副作用 |
